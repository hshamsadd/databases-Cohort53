# Databases Week 2: Easy Guide to Data Management
...
3. Open your terminal or command prompt and run: 
   ```bash
   node `Talking_to_Databases.js`
4.  Go to pgAdmin 4 and refresh Databases you shall find there the `demo_db` database with the `my_test_table` table created inside `schemas` under `tables` with value of `Hello from Node.js!`.
...
1.  **Try it yourself:** In the `Talking_to_Databases.js` file, what would happen if you swapped the order of `createTableInDatabase(client)` and `insertDataIntoDatabase(client)`? Try it and see what error you get. Answer: An error as you can't insert before creating the table.
