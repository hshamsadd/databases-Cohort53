...
CREATE TABLE recipes (
  ...
  name VARCHAR(200) NOT NULL   -- 2NF: fully depends on id, 3NF: no transitive dependency
);

-- Table to store categories
CREATE TABLE categories (
  id SERIAL PRIMARY KEY,       -- 1NF: atomic, unique identifier
  name VARCHAR(100) NOT NULL   -- 2NF: depends only on id, 3NF: no transitive dependency
);

-- Link table to connect recipes to categories (junction table)
CREATE TABLE recipe_categories (
  ...
  -- 2NF: no partial dependency (composite PK, no extra columns)
...
CREATE TABLE ingredients (
  id SERIAL PRIMARY KEY,       -- 1NF: atomic, unique identifier
  name VARCHAR(150) NOT NULL   -- 2NF: depends on id, 3NF: no transitive dependency
);

-- Link table to connect recipes to ingredients (junction table)
CREATE TABLE recipe_ingredients (
  ...
  -- 1NF, 2NF, 3NF satisfied (same logic as recipe_categories)
);
...
CREATE TABLE steps (
  ...
  step_number INT NOT NULL,     -- depends on recipe_id
  instruction TEXT NOT NULL,    -- atomic instruction
  FOREIGN KEY (recipe_id) REFERENCES recipes(id)
  -- 1NF: atomic columns
  -- 2NF: non-key attributes depend on full PK (id)
  -- 3NF: no transitive dependency (instruction does not depend on step_number, only on id)
);
...
-- My database was already in 3NF.
-- What changes did you have to do to normalize your database?
-- The only normalization improvements I applied were adding foreign key constraints to enforce relationships explicitly.